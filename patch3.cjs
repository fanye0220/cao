...
// We want to replace the buttons section which starts with:
/*
                           ) : (
                               <>
                                   <button
...
const targetRegex = /\\) : \\(\\s*<>\\s*<button\\s*onClick=\\{\\(\\) => \\{\\s*setTempFirstMes\\(getCurrentMessage\\(\\)\\);\\s*setIsEditingFirstMes\\(true\\);\\s*\\}\\}/;
...
const renderRegex = /<textarea\\s*readOnly=\\{\\!isEditingFirstMes\\}\\s*value=\\{isEditingFirstMes \\? tempFirstMes : getCurrentMessage\\(\\)\\}\\s*onChange=\\{\\(e\\) => isEditingFirstMes && setTempFirstMes\\(e\\.target\\.value\\)\\}\\s*className=\\{.*?\\}\\s*placeholder="暂无内容\\.\\.\\."\\s*\\/>/s;
