...
function App() {
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      const updated = await Promise.all(characters.map(async (char) => {
        try {
          const blob = await loadImage(char.id);
          if (blob) return { ...char, avatarUrl: URL.createObjectURL(blob) };
        } catch (e) {}
        return char;
      }));
      setCharacters(prev => {
        const urlMap = new Map(updated.map(c => [c.id, c.avatarUrl]));
        return prev.map(c => urlMap.has(c.id) ? { ...c, avatarUrl: urlMap.get(c.id)! } : c);
      });
    };
  ...
  return (
    ...
    >
      {/* Decorative Orbs */}
      <div className={`absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] pointer-events-none transition-all duration-700 ${theme === 'light' ? 'bg-blue-300/30' : 'bg-blue-500/20'}`} />
      <div className={`absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] pointer-events-none transition-all duration-700 ${theme === 'light' ? 'bg-purple-300/30' : 'bg-purple-500/20'}`} />
